 ...
 * the same time.
 * <p>
 * 一个抽象类,说明它内部有一些抽象方法需要子类实现去定制一些特制的功能。
 */
public abstract class MultithreadEventExecutorGroup extends AbstractEventExecutorGroup {
    ...
    // 中断执行的返回结果，因为需要关闭时一个执行组所以为了异步执行所以返回了一个应答然后根据用于调用去决定是等待获取结果，还是去设置一个结果事件
    ...
    protected MultithreadEventExecutorGroup(int nThreads, Executor executor,
        ...
        for (int i = 0; i < nThreads; i ++) {
            ...
            try {
                ...
                // 如果success是false，则代表创建失败则将已经创建好的执行器进行关闭
                if (!success) {
                    for (int j = 0; j < i; j ++) {
    ...
    /**
     * 迭代执行器的时候调用的是只读的set（readonlyChildren）
     */
    @Override
     ...
     *
     * <p>声明了一个创建执行器的方法并且抽象的，因为每个执行器的实现都有特殊的操作所以此处抽象</p>
     */
    protected abstract EventExecutor newChild(Executor executor, Object... args) throws Exception;

    /**
     * 之前说过调用线程组的关闭其实就是遍历执行器集合的关闭方法
     * 因为之前加了监听器去处理返回结果所以此处返回的future用于监听是否执行结束了
     */
    @Override
    ...
    /**
     * 只有所有的执行器都是关闭中ShuttingDown状态才会是true
     */
    @Override
    ...
    public boolean awaitTermination(long timeout, TimeUnit unit)
        ...
        loop: for (EventExecutor l: children) {
            for (;;) {
                ...
                // 如果小于等于0则跳出loop就是最外层循环，不再循环
                if (timeLeft <= 0) {
                    break loop;
                }
                // 否则当前的线程等待计算的时间，如果在时间内终止，则跳出循环再次遍历下一个执行器，然后计算时间再次重复操作
